    ...
    public class SuggestionController : ControllerBase
    {
        ...
        private async Task<List<Song>> GetRecentlyUploaded()
        {
            ...
            foreach (var r in queryResult)
            {
                ...
                var tags = (await _database.Call($"SELECT * FROM songtags WHERE id={r["id"]}")).Select<dynamic, string>(e => e["tag"]).ToList();
                result.Add(new Song
                {
                    Id = r["id"],
                    Name = r["name"],
        ...
        private async Task<List<Song>> GetRecentlyPlayed()
        {
            ...
            var ids = (await _database.Call(
                $"SELECT song FROM recents WHERE username='{Username}' ORDER BY added_date DESC LIMIT 15")).Select<dynamic, long>(e => e["song"]);
            foreach (var id in ids)
            {
                ...
                var tags = (await _database.Call($"SELECT * FROM songtags WHERE id={song["id"]}")).Select<dynamic, string>(e => e["tag"]).ToList();
                result.Add(new Song
                {
                    Id = song["id"],
                    Name = song["name"],
        ...
        private async Task<List<Song>> GetTop50()
        {
            var result = new List<Song>();
            var ids = (await _database.Call(
                $"SELECT song FROM favorites ORDER BY added_date DESC LIMIT 15")).Select<dynamic, long>(e => e["song"]);
            foreach (var id in ids)
            {
                var song = (await _database.Call($"SELECT * FROM song WHERE id={id}")).First();
                var favorites = (await _database.Call($"SELECT * FROM favorites WHERE songid={song["id"]}")).Count;
